...
(defn format-input [s]
 ...
 (defn left-right [d]
  ...
  (let [frequency-right (frequencies right)
        how-often (select-keys frequency-right left)
        result (->> how-often
                    (map (fn [[k v]] (* k v)))
                    (apply +))]
    result))
...
(defn -main
  ...
  [& args] 
  (->> (format-input input) left-right similarity-score prn))
